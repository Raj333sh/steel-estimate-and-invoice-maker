  ...
  items.forEach((item, index) => {
    ...
    itemDiv.innerHTML = `
                    ...
                    <div class="input-grid">
                        ...
                        ${
                          ["Custom Materials"].includes(item.materialType)
                            ? `
                        <div class="input-group">
                        ...
                        <div class="input-group">
                            <label>Length *</label>
                            <input type="number" step="0.01" placeholder="e.g., 6.00" value="${
                              item.length
                            }" 
                                   oninput="updateItem(${
                                     item.id
...
function generateEstimate() {
  ...
  document.getElementById("displayOrgName").textContent = orgName || "N/A";
  document.getElementById("displayAddress").textContent = address || "N/A";
  document.getElementById("displayContact").textContent = contact || "N/A";
  ...
  document.getElementById("displayCustomerNotes").textContent = customerNotes;
  ...
  items.forEach((item) => {
    ...
    const isCustomMaterial = item.materialType === "Custom Materials";

    if (isCustomMaterial) {
      if (!item.customDescription || !item.quantity || !item.costPerUnit) {
        hasError = true;
        return;
      }
    ...
    if (item.materialType !== "Custom Materials") {
    ...
    if (item.materialType === "Custom Materials") {
      ...
      totalCost =
        parseFloat(item.costPerUnit || 0) * parseFloat(item.quantity || 0);
    } else {
    ...
    calculations.push({
      ...item,
      displayname: isCustomMaterial
        ? item.customDescription
        : item.materialType,
      sectionSize: isCustomMaterial ? "N/A" : item.sectionSize,
      weightPerMeter: isCustomMaterial ? "N/A" : sectionData.weight,
...
function displayResults(calculations) {
  ...
  calculations.forEach((item, index) => {
    ...
    row.innerHTML = `
                    ...
                    <td>${
                      item.materialType === "Custom Materials"
                        ? "-"
                        : item.costPerKg || "-"
                    }</td>
                    <td>${
  ...
  let totalGST = calculations.reduce((sum, item) => {
    return sum + parseFloat(item.gstAmount);
  }, 0);

  tfoot.innerHTML = `
                <tr>
                    <td colspan="12" style="text-align: right;"><strong>Gst Amount (18%):</strong></td>
                    <td></td>
                    <td></td>
                    <td></td>
